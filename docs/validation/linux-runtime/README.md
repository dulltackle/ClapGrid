...
对应 [#13](https://github.com/dulltackle/ClapGrid/issues/13)，2026-09-26。首轮按用户要求只做实际运行验证，不新增自动化测试，不修改原型服务代码。后续已交付 [Ubuntu 安装入口与更新插件](delivery/README.md)，该文档记录新增代码、安装缓存及实测结果。**事项尚未全部验收，不关闭。**
...
首次安装由 Codex 检查 Python，缺失时解释所需安装内容，并通过宿主批准的执行方式安装。APT 需要系统管理员权限；本轮仅在专用容器内以 root 安装，没有改动宿主包。普通桌面用户如需 sudo/系统认证，必须使用宿主允许的审批和认证流程，不能读取或代填密码，不能悄悄改用其他提权路径。首轮插件 0.1.0 只会在缺少 Python 时请求用户安装；后续已增加显式 install 入口并更新安装缓存，详见 [安装入口交付](delivery/README.md)。系统管理员认证与宿主执行审批仍须分别满足。

日常操作由 Codex 从安装缓存运行 Python 启动器，并用宿主 `open_in_codex` 的 right/browser 打开返回的本机 URL。用户无需手动打开终端。该流程在已有组件的当前宿主已运行；首次安装入口现已交付并完成容器实测，缺少 Python 的真实 Codex 桌面首装仍待验收。
...
宿主审批允许与真实拒绝均已验证。新聊天“测试宿主审批拒绝”中，用户允许只读准备检查后拒绝单独的 start 请求；宿主返回 `rejected by user`，代理明确反馈本次操作未执行，随后没有工具调用或变换执行方式。本聊天接续只读查询仍返回连接拒绝。详见 [审批拒绝证据](delivery-evidence/approval-denial.json) 与 [后续状态](delivery-evidence/denial-followup-status.txt)。第一次误拒绝准备检查不计入启动拒绝验收。
...
本轮用户收到完全退出并重开的操作指引后回复“已重开”。代理随后首先独立读取 `/state`，没有运行启动器。退出前最后一次状态（上一轮工具输出，现按原字段归档为 [最终退出前基线](host-before-exit-final.json)）为 PID `154611`、任务 `f7baf5d4-a38a-437f-a6bc-d426aa4e2e9d`、运行中 `282/900`、locked=true、时间 `1790408584.7747164`；[重开后首次状态](host-after-exit.json) 为同一 PID、同一任务、已完成 `900/900`、locked=false、时间 `1790409223.504856`。两次观测相隔约 639 秒，基线时任务仍有 618 秒未完成。

据用户的退出重开确认及 API 前后证据，本轮退出后同一服务、同一任务继续验证通过。用户未提供精确退出和重开时刻，两个时间戳是 API 观测时刻，不是桌面进程退出事件的自动采集。该证据独立于 #10 的旧版本结果。容器实验已停止并保留；宿主服务目前空闲保留供右侧页面查看，可按下列命令正常退出。
...
| 安装与分发方式、Codex 日常启动与右侧打开 | Ubuntu 模拟原型已交付：系统 Python 安装入口与可构建插件 ZIP，新安装缓存启动及右侧打开通过；后续独立聊天已读取新版本技能 |
| 宿主审批允许及拒绝反馈 | 通过：允许启动已有证据，真实 start 审批拒绝及可理解反馈已补齐 |
| 独立 state、退出后继续、服务与任务复用 | 通过：独立请求与复用证据、用户退出重开确认、同 PID/任务从 282/900 到完成 |
| 可复现步骤、实际版本和运行交付说明 | 已补齐本轮步骤、版本、原始证据及限制 |

未新增自动化测试；仓库没有类型检查或全量测试配置，原型服务未修改，后续新增安装入口；验证清单见安装入口交付说明。提交前检查证据 JSON、归档 ZIP/缓存脚本校验值、文档引用和 Git 空白错误，并进行只读审查。
